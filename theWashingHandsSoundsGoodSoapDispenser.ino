...
int tempo = 150; // Duration: 28 seconds. Tempo = 108 is the best, but the duration is too long for this project (40 seconds).

// Speaker pin defined.
int speaker = 3;

// Button on soap dispenser defined.
int soapDispenser = 12;

// Here is the melody of the song to play when the soap dispenser (soapDispenser) is pressed.
...
// Funtion to play the song (melody[]).
void song(){
...
void loop(){
  //
  int buttonState = digitalRead(soapDispenser);
  
  if(buttonState == HIGH){
    // If soapDispenser (button) is ON, is pressed, then play song.
    song();

    // Set soapDispenser (button) OFF.
    buttonState = LOW;
    }
}
